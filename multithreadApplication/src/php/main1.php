    ...
    if ($username && $password) {
        ...
        if ($numrows > 0) {
             while ($row = mysqli_fetch_assoc($result)) {
                ...
                $query1   = ("SELECT zone_name FROM railway_zone WHERE zone_code ='$zone'");
...
if (isset($_POST['approve'])) {
    ...
    $sql = "UPDATE railways_register SET railway_account_status = '$status' where railway_email = '$email' and railway_uid = '$railway_uid'";
    $res = mysqli_query($conn, $sql);
    if ($res) {
        $body = "Congaratulation your account has been verified by the state.
        You can login using following link:https://internship.aicte-india.org/internshipinindianrailways/railway_login.php?id=1";
...
if (isset($_POST['disapprove'])) {
    ...
    $email = mysqli_real_escape_string($conn, $_POST['email']);
    $railway_uid = mysqli_real_escape_string($conn, $_POST['railway_uid']);
    $sql = "UPDATE railways_register SET railway_account_status = '$status' where railway_uid = '$railway_uid'";
    $res = mysqli_query($conn, $sql);
    if ($res) {
...
if (isset($_POST['delete_project'])) {
    $id = mysqli_real_escape_string($conn, $_POST['id']);
    $sql = "Delete from railway_project where id = '3'";
    $res = mysqli_query($conn, $sql);
    if ($res) {
        echo "success";
    }
