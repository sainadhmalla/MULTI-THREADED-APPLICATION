<?php include './includes/auth_state.php';
...
<section class="m-4">
    <div id="to_be_approved">
	 ...
	 echo '<h1 class="font-weight-bold display-4 mb-4">
        ...
        $sql = "SELECT * FROM railways_register where railway_state_status = '1' and railway_organisation = 'Division' and railway_zone = '$zone' or railway_state_status = '1' and railway_organisation = 'Workshop' and railway_zone = '$zone'";
        }
        else if($_SESSION['type']=='Division' || $_SESSION['type'] == 'Workshop')
        {
 echo '<h1 class="font-weight-bold display-4 mb-4">
           Division/Workshop Level Railway Approval Dashboard
       </h1>';
        $zone = $_SESSION['zone_id'];
        $division = $_SESSION['division'];
        $sql = "SELECT * FROM railways_register where railway_state_status = '1' and railway_zone = '$zone'and railway_organisation = 'Department' and railway_organization='$division'";
        }
        else{
            $sql = "SELECT * FROM railways_register where railway_state_status = '1' and railway_organisation = 'Zone'";  
        }
            ...
            while ($row = mysqli_fetch_assoc($res)) {
                echo '<div class="col mb-4">
                    ...
                    <div class="card-body">
                        ...
                        if($_SESSION['type']=='Zone'){
                            echo' <h6 class="card-subtitle mb-2 text-muted disable-select">Divison/Workshop Name: ' . $row['railway_organization'] . '</h6>';
                            }
                            else if($_SESSION['type']=="Division" || $_SESSION['type'] == 'Workshop'){
                                echo '
                        ...
                        <button  type="submit"class="btn btn-success disable-select" name="approve" id="apprve" disabled>Approve</button>
